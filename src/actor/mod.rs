...
impl NetworkActor {
    ...
    pub fn push_entity(&mut self, entity: Entity) {
        self.entities_buffer.push(entity);
    }

    pub fn drain_entities(&mut self) -> ::std::vec::Drain<Entity> {
        self.entities_buffer.drain(..)
    }

    // Private method only intended to use by variants of Actors
    pub fn new(id: ActorId, client: Client) -> NetworkActor {
        NetworkActor {
            id: id,
            entities: HashSet::new(),
            entities_buffer: Vec::new(),
            client: client,
